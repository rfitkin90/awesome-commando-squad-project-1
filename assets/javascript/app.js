            ...
            .then(function (response) {
                ...
                for (var i = 0; i < 10; i++) {
                    ...
                    $(`#result-${response.data.events[i].id}`).append(`<button class='btn info-btn'
                        ...
                        data-date='${response.data.events[i].datetime_utc}'>More Info</button>`);
                }
            });
        ;
    });
...
// if (Math.abs(moment().diff(moment($(this).attr('data-date')), 'd')) <= 5) {
    ...
    // if (moment($(this).attr('data-date')).diff(moment(reponse.data.list.length[reponse.data.list.length - 1].dx_txt))) {
